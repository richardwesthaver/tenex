...
/// This type is used as type parameter to the following functions, when `rq` is
/// `None`.
...
/// The Content-Type header is set automatically to application/json. Also
...
/// An ongoing download.
///
/// Note that this does not necessarily result in a download. It is returned by
/// all API methods that are capable of downloading data. Whether a download
/// takes place is determined by the `Content-Type` sent by the server;
/// frequently, the parameters sent in the request determine whether the server
/// starts a download (`Content-Type: whatever`) or sends a response
  ...
  Download<'a, Request, Response>
{
  ...
  /// If the server responded with a `Response` object, no download is
  /// started; the response is wrapped in the `DownloadResult<Response>`
  /// object.
  ///
  /// Whether a download takes place or you receive a structured `Response`
  /// (i.e. a JSON object) depends on the `Content-Type` sent by the server.
  /// It is an error to attempt a download without specifying `dst`. Often,
  /// whether a download takes place is influenced by the request parameters.
  /// For example, `alt = media` is frequently used in Google APIs to indicate
  /// that a download is expected.
  pub async fn do_it(
    &mut self,
    ...
    loop {
      ...
      if let Some(rq) = self.rq.take() {
        body = hyper::Body::from(serde_json::to_string(&rq).context(format!("{:?}", self.rq))?);
      } else {
        body = hyper::Body::from("");
      }
      ...
      if status.is_success() {
        ...
        if let Some(dst) = dst {
          ...
          return Err(
            ApiError::DataAvailableError(format!(
      ...
      // Server redirects us.
        ...
        if new_location.is_none() {
          return Err(
            ApiError::RedirectError(format!("Redirect doesn't contain a Location: header")).into(),
          );
        }
...
impl<'client, Response: DeserializeOwned> ResumableUpload<'client, Response> {
  ...
  /// Upload data from a reader; use only if the reader cannot be seeked. Memory
  /// usage is higher, because data needs to be cached if the server hasn't
  /// accepted all data.
  pub async fn upload<R: tokio::io::AsyncRead + std::marker::Unpin>(
    ...
    loop {
      ...
      if !status.is_success() && status.as_u16() != 308 {
        ...
        return Err(ApiError::HTTPResponseError(status, status.to_string())).context(body_to_str(
      ...
      if current >= size {
        ...
        if !status.is_success() {
          return Err(
            Error::from(ApiError::HTTPResponseError(
              status,
  ...
  /// Upload content from a file. This is most efficient if you have an actual
  /// file, as seek can be used in case the server didn't accept all data.
