...
pub async fn upload_file(cl: &drive::FilesService, f: &Path) -> anyhow::Result<()> {
  ...
  // get() returns a response type (Response(...)) if the server returned an
  // application/json response, otherwise it will download to a Writer. If you
  // don't supply a Writer and there is data to download, an error is returned
  // (ApiError::DataAvailableError).
  match get_file {
  ...
  // Get file metadata. We don't specify &alt=media, so no download will take
  // place.
  let mut params = drive::FilesGetParams::default();
  params.file_id = file_id.clone();
