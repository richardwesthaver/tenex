...
pub struct AboutStorageQuota {
  /// i64: The usage limit, if applicable. This will not be present if the user
...
pub struct Channel {
  ...
  /// i64: Date and time of notification channel expiration, expressed as a Unix
...
pub struct ContentRestriction {
  ...
  /// DateTime: The time at which the content restriction was set (formatted RFC
...
pub struct File {
  ...
  /// About resource. If an unsupported color is specified, the closest color
  ...
  /// unless a new revision is uploaded. If a file is created with a Google
  /// Doc MIME type, the uploaded content will be imported if possible. The
  ...
  /// i64: The number of storage quota bytes used by the file. This includes the
  ...
  /// i64: The size of the file's content in bytes. This is applicable to binary
  ...
  /// i64: A monotonically increasing version number for the file. This reflects
...
pub struct Permission {
  ...
  /// date-time). Expiration times have the following restrictions:
...
pub struct Revision {
  ...
  /// i64: The size of the revision's content in bytes. This is only applicable
...
pub struct PermissionsCreateParams {
  ...
  /// and the request is attempting to transfer the ownership of the item. If
  /// set to true, the item will be moved to the new owner's My Drive root
  /// folder and all prior parents removed. If set to false, parents are not
  /// changed.
...
impl AboutService {
  /// Create a new AboutService object. The easiest way to call this is wrapping
...
impl FilesService {
  /// Create a new FilesService object. The easiest way to call this is wrapping
  /// the Authenticator into an `Arc`: `new(client.clone(),
  /// Arc::new(authenticator))`. This way, one authenticator can be shared
  /// among several services.
  ...
  pub async fn update_resumable_upload<'client>(
    ...
    let opt_request = Some(req);
    let (_resp, headers): (EmptyResponse, hyper::HeaderMap) =
      do_request_with_headers(&self.client, &full_uri, &headers, "PATCH", opt_request).await?;
    if let Some(dest) = headers.get(hyper::header::LOCATION) {
      Ok(ResumableUpload::new(
        hyper::Uri::try_from(dest.to_str()?)?,
        &self.client,
